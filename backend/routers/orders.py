...
from schemas.orders import OrderCreateIn, ReceiveOrderIn, ReceivingConfirmIn, ReceivingDraftIn, ReviewIn
...
from services.delivery_slot import DELIVERY_SLOT_RE, is_one_hour_delivery_slot
...
def _ordered_qty_kg_for_line(order: Order, line_index: int) -> tuple[float, str]:
    ...
    unit = str(snap.get("unit") or item.get("unit") or "kg").strip().lower()
    if "斤" in unit:
        return round(qty * 0.5, 6), str(snap.get("unit") or item.get("unit") or "斤")
    return round(qty, 6), str(snap.get("unit") or item.get("unit") or "kg")
...
async def _all_receiving_lines_confirmed(db: AsyncSession, order: Order) -> bool:
    ...
    for i in range(1, n + 1):
        row = by_index.get(i)
        if not row or str(row.status) != "confirmed" or row.confirmed_kg is None:
            return False
    return True
...
async def _supplier_printed_allocation_ids(
    db: AsyncSession, order_id: int, supplier_user_id: int
) -> set[int]:
    logs = (
        await db.scalars(
            select(AuditLog)
            .where(
                AuditLog.actor_user_id == supplier_user_id,
                AuditLog.action == "supplier_print_allocation_label",
                AuditLog.object_type == "order",
                AuditLog.object_id == order_id,
            )
    ...
    for row in logs:
        ...
        aid = int(data.get("allocation_id") or 0)
        if aid > 0:
            printed_ids.add(aid)
    return printed_ids
...
async def _supplier_label_gate_state(
    ...
    my_alloc_ids_int = [int(i) for i in my_alloc_ids]
    printed_line_ids = await _supplier_printed_allocation_ids(db, order_id, supplier_user_id)
    line_total = len(my_alloc_ids_int)
    line_printed_count = sum(1 for i in my_alloc_ids_int if i in printed_line_ids)
    ...
    if order_label_printed and line_total > 0:
        # 整单标签打印成功视为该供货商全部行级标签已打印（全选语义）
        printed_line_ids = set(my_alloc_ids_int)
        line_printed_count = line_total
    all_line_printed = bool(line_total > 0 and line_printed_count == line_total)
    if line_total > 0:
        can_ship_by_print = bool(order_label_printed or all_line_printed)
    else:
        can_ship_by_print = bool(order_label_printed)
    return {
        ...
        "printed_line_ids": sorted(int(i) for i in printed_line_ids),
    }
...
async def create_order(
    ...
    if not DELIVERY_SLOT_RE.match(slot_s):
        raise HTTPException(400, "配送时段格式非法，须为整点区间，如 06:00-07:00")
    if not is_one_hour_delivery_slot(slot_s):
        raise HTTPException(
            400,
            "配送时段须为恰好 1 小时（整点起止），例如 06:00-07:00；当日最后一档可为 23:00-24:00",
        )
    delivery_address = (payload.delivery_address or "").strip()
    if not delivery_address:
        raise HTTPException(400, "请填写配送地址")
    delivery_lng = float(payload.delivery_lng) if payload.delivery_lng is not None else None
    delivery_lat = float(payload.delivery_lat) if payload.delivery_lat is not None else None
    if (delivery_lng is None) != (delivery_lat is None):
        raise HTTPException(400, "经纬度需要同时填写或同时留空")
...
async def save_receiving_line_draft(
    ...
    row.draft_kg = float(body.net_kg)
    ...
    await write_audit_log(
        ...
        object_type="order",
        object_id=order.id,
        detail=f"行{line_index}称重草稿 net_kg={body.net_kg}",
        **_audit_meta(request),
    )
    await db.commit()
    return {"message": "ok", "line_index": line_index, "draft_kg": float(body.net_kg)}
...
async def confirm_receiving_line(
    ...
    if is_shortage:
        if not body.shortage_reason:
            raise HTTPException(
                400,
                "实收重量少于下单折算重量时，请选择少收原因：缺货(lack)、质量问题(quality)、其他(other)",
            )
        row.shortage_reason_code = str(body.shortage_reason.code)
        row.shortage_reason_detail = (
            (body.shortage_reason.detail or "").strip() if body.shortage_reason.code == "other" else None
        )
        row.shortage_ordered_kg = ordered_kg
        row.shortage_delta_kg = round(ordered_kg - received, 4)
    else:
        row.shortage_reason_code = None
        row.shortage_reason_detail = None
        row.shortage_ordered_kg = None
        row.shortage_delta_kg = None
    row.confirmed_kg = received
    row.draft_kg = received
    ...
    await write_audit_log(
        ...
        object_type="order",
        object_id=order.id,
        detail=f"行{line_index}确认称重 confirmed_kg={body.net_kg} shortage={is_shortage}",
        **_audit_meta(request),
    )
    await db.commit()
...
async def order_detail(
    ...
    if user.role == "supplier":
        ...
        printed_line_ids = set(int(i) for i in (label_gate.get("printed_line_ids") or []))
        for row in allocations:
            if int(row.get("supplier_id") or 0) != int(user.id):
                continue
            row["line_label_printed"] = bool(int(row.get("id") or 0) in printed_line_ids)
...
async def print_label(
    ...
    if not await _supplier_can_act_on_order(db, order, user.id):
        raise HTTPException(403, "无权限操作该订单")
    record = await _supplier_sort_record(db, order_id, user.id)
    if not record:
        record = SortRecord(
            order_id=order_id,
            operator_id=user.id,
    ...
    await write_audit_log(
        db=db,
        actor_user_id=user.id,
        action="supplier_print_order_label",
        category="order",
        object_type="order",
        object_id=order.id,
        detail=f"供货商调用第三方API打印整单标签并视为分拣完成 {order.order_no}",
        after_json={"label_printed": True},
    )
    await db.commit()
    return {"message": "整单标签打印成功，已视为分拣完成", "sort_done": True}
...
async def print_allocation_label(
    ...
    alloc = await db.scalar(
        select(OrderItemAllocation).where(
            OrderItemAllocation.id == allocation_id,
            OrderItemAllocation.order_id == order_id,
            OrderItemAllocation.supplier_id == user.id,
        )
    )
    if not alloc:
        raise HTTPException(404, "分包明细不存在或未分包给当前账号")
    await write_audit_log(
        db=db,
        actor_user_id=user.id,
        ...
        object_type="order",
        object_id=order.id,
        detail=f"供货商调用第三方API打印行级标签 allocation={allocation_id}",
        after_json={
...
async def ship_order(
    ...
    label_gate = await _supplier_label_gate_state(db, order.id, user.id)
    if not bool(label_gate["order_label_printed"]) and not bool(label_gate["all_line_labels_printed"]):
        remaining = int(label_gate["line_label_total"]) - int(label_gate["line_label_printed_count"])
        raise HTTPException(
            400,
            f"请先打印整单标签，或完成全部行级标签打印后再发货（当前剩余{max(remaining, 0)}行）",
        )
...
async def receive_order(
    ...
    use_receiving = await _has_receiving_lines(db, order.id)
    if use_receiving:
        if not await _all_receiving_lines_confirmed(db, order):
            raise HTTPException(400, "尚有订单明细未完成称重确认")
        wh = (body.warehouse_signature or "").strip()
        cr = (body.carrier_signature or "").strip()
