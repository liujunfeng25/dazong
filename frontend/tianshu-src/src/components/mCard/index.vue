<template>
  <div class="m-card" :style="calcWidthHeightStyle">
    <div class="m-card-hd">
      ...
      <div class="m-card-hd-title-row">
        <div class="m-card-hd-title">{{ title }}</div>
        <span
          v-if="titleHint"
          class="m-card-hd-hint"
          :title="titleHint"
          role="img"
          aria-label="指标说明"
        >?</span>
    ...
    <div class="m-card-bd" :style="calcWidthHeightStyle">
      ...
      <div class="m-card-bd-content">
        <div
          v-if="overlayLoading"
          class="m-card-bd-loading"
          aria-busy="true"
          aria-live="polite"
        >
          <span class="m-card-bd-loading__text">数据更新中…</span>
        </div>
        <slot></slot>
      </div>
    </div>
...
.m-card {
  ...
  &-hd {
    ...
    &-hint {
      ...
      border: 1px solid rgba(129, 224, 255, 0.55);
      border-radius: 50%;
      background: rgba(12, 48, 72, 0.65);
      cursor: help;
      box-shadow: 0 0 8px rgba(79, 197, 221, 0.25);
      -webkit-text-fill-color: rgba(186, 242, 255, 0.95);
    }
    .saoguang {
      position: absolute;
      left: 0;
      ...
      img {
        width: 89px;
        height: 36px;
        animation: saoguangMove 3.4s linear infinite;
      }
    }
  }
  &-bd {
